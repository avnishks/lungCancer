...
def resampleSitk(imgSitk, mskSitk):
  global newSpacing
  global newSize
  ...
  return imgSitk, mskSitk, origSize, origSpacing
...
def savePNG(patientID, imgSitk, mskSitk):
...
def saveNrrd(patientID, imgSitk, mskSitk):
    imgFile = os.path.join(imageOutput, patientID+'_img.nrrd')
    mskFile = os.path.join(imageOutput, patientID+'_tum.nrrd')
...
def runCore(patients, coreNr, resultDict):
  ...
  for patientID in patients.keys():
    ...
    imgSitk, mskSitk, origSize, origSpacing = resampleSitk(imgSitk, mskSitk)
    ...
    saveNrrd(patientID, imgSitk, mskSitk)
    
    if PNG:
      savePNG(patientID, imgSitk, mskSitk)
    
    resultDict[patientID] = [imgFile, mskFile,
...
if __name__== "__main__":
  ...
  for file in files:
    if 'img' in file:
      ...
      imgFile = file
      mskFile = imgFile.replace('img', 'tum')
